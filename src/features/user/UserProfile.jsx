      ...
      <div className="profile-container">
        <aside className="profile-sidebar">
          ...
          <nav className="profile-nav">
            <a
              href="#statistics"
              className={`profile-nav-item ${currentMenu === 'statistics' ? 'active' : ''}`}
              onClick={(e) => { e.preventDefault(); setCurrentMenu('statistics'); }}
            >
            ...
            <a
              href="#vouchers"
              className={`profile-nav-item ${currentMenu === 'vouchers' ? 'active' : ''}`}
              onClick={(e) => { e.preventDefault(); setCurrentMenu('vouchers'); }}
            >
              <span className="nav-icon">🎟️</span> Mã giảm giá
            </a>
            <a
              href="#membership"
              className={`profile-nav-item ${currentMenu === 'membership' ? 'active' : ''}`}
              onClick={(e) => { e.preventDefault(); setCurrentMenu('membership'); }}
            >
              <span className="nav-icon">👑</span> Hạng thành viên
            </a>
            <a
              href="#personal-info"
              className={`profile-nav-item ${currentMenu === 'personal-info' ? 'active' : ''}`}
              onClick={(e) => { e.preventDefault(); setCurrentMenu('personal-info'); }}
            >
              <span className="nav-icon">👤</span> Thông tin cá nhân
            </a>
            <a
              href="#password"
              className={`profile-nav-item ${currentMenu === 'password' ? 'active' : ''}`}
              onClick={(e) => { e.preventDefault(); setCurrentMenu('password'); }}
            >
              ...
              <div className="order-list">
                {loadingOrders ? (
                  ...
                  filteredOrders.map((order) => (
                    <div
                      key={order.id}
                      className={`order-card ${order.status === 'Chờ xử lý' ? 'highlight' : ''}`}
                    >
                      <div className="order-status-right">
                        Trạng thái:{' '}
                        <span style={{
                          color:
                            order.status === 'Thành công' ? '#2ecc71' :
                            order.status === 'Chờ xử lý' ? '#f39c12' :
                            order.status === 'Đã hủy' ? '#e74c3c' : '#3498db',
                          fontWeight: 'bold',
                          marginLeft: 6
                        }}>
                          {order.status}
                      ...
                      <div className="order-details">
                        ...
                        <div className="order-info-center">
                          <h4 className="order-product-name" style={{ marginBottom: 8 }}>
                            {order.productName}
                          </h4>
                          {order.extraCount > 0 && (
